...
### 3.2. 録画制御機能
- **機能ID:** AND-FUNC-002
- **概要:** 物理ボタンを使用して、動画の録画開始・停止を制御する。
- **物理ボタン割り当て:**
  - **ボタン1 (短押し):** 録画開始
  - **ボタン2 (短押し):** 録画停止
- **状態遷移:**
  - `待機状態` で `ボタン1` を押下 → `録画中状態` へ遷移。
  - `録画中状態` で `ボタン2` を押下 → `待機状態` へ遷移。
- **処理フロー (録画開始):**
  1. `待機状態` であることを確認する。
  2. カメラリソースを初期化し、録画設定（解像度、フレームレート）を適用する。
  3. ファイル名（例: `VID_{yyyyMMdd_HHmmss}.mp4`）を決定し、ストレージに空ファイルを作成する。
  4. 録画を開始する。
...
### 3.3. デバイス状態通知機能
- **機能ID:** AND-FUNC-003
- **概要:** 音声と振動を用いて、デバイスの様々な状態をユーザーにフィードバックする。
- **物理ボタン割り当て:**
  - **ボタン3 (長押し):** 現在の状態（録画中/待機中）、バッテリー残量、ネットワーク状態、ストレージ残量を順に音声通知する。
  - **ボタン3 (2回短押し):** 音量を上げる。
  - **ボタン3 (1回短押し):** 音量を下げる。
- **自動通知トリガーと内容:**
  - **バッテリー残量低下:**
...
### 3.5. ビーコンによる録画エリア制御
- **機能ID:** AND-FUNC-005
- **概要:** BLEビーコンの電波強度を利用して、録画可能エリア内外を判定し、録画を制御する。
...
- **処理フロー:**
  ...
  3. `待機状態（エリア内）` のときに、ビーコンが5回連続で検出されなくなった場合:
     a. `待機状態（エリア外）` に遷移する。（ユーザーへの通知は不要）
  4. `待機状態（エリア外）` のときに、ビーコンが3回連続で検出された（エリア内に戻ったと判定）場合:
     a. 「作業エリアに入りました。録画が可能です」と音声通知する。
     b. `待機状態（エリア内）` に遷移する。
- **異常系処理:**
...
sequenceDiagram
    ...
    User->>AndroidOS: 戻る/ホームボタン押下
    AndroidOS->>Device: (操作はアプリに捕捉され、終了しない)

    opt アップロード条件成立 (充電中 & Wi-Fi接続など)
        Device->>PC: ファイル転送を開始
        PC-->>Device: 転送完了
        Device->>Device: 転送済みファイルを削除
    end
    User->>Device: ボタン3長押し (状態確認)
    Device-->>User: 音声「録画状態はXX、バッテリーXX%、Wi-Fi接続中...」
    Note right of Device: 状態: 待機状態(エリア内)

    User->>Device: ボタン1押下 (録画開始)
    Device-->>User: 音声/振動「録画を開始します」
    Note right of Device: 状態: 録画中状態

    alt 通常の録画停止
        User->>Device: ボタン2押下 (録画停止)
        Device-->>User: 音声/振動「録画を停止しました」
        Note right of Device: 待機状態(エリア内)に転移
    else エリア外での強制録画停止
        User->>Device: (エリア外へ移動)
        Device->>Device: エリア外を検知
        Device-->>User: 音声「作業エリア外に出ました。録画を停止します」
        Device->>Device: 録画を強制停止
        Note right of Device: 待機状態(エリア外)に転移
    end
    Note right of Device: 状態: 待機状態(エリア外)
    User->>Device: ボタン1押下 (録画開始試行)
    Device-->>User: (操作無効)

    User->>Device: (エリア内に復帰)
    Device->>Device: エリア内を検知
    Device-->>User: 音声「作業エリアに入りました」
...
なお、どの状態においても、ボタン3を長押しすることでデバイスの状態（バッテリー、ネットワーク等）を音声で確認できます。この操作は本状態遷移に影響を与えません。

```mermaid
stateDiagram-v2
    [*] --> 待機状態（エリア内）: アプリ起動
    待機状態（エリア内） --> 録画中状態: ボタン1押下
    録画中状態 --> 待機状態（エリア内）: ボタン2押下

    録画中状態 --> 待機状態（エリア外）: ビーコン圏外へ
    note right of 録画中状態
        録画中に作業エリアから出た場合、
        録画を強制停止し、録画が禁止される
        「待機状態（エリア外）」へ遷移する。
    ...
    待機状態（エリア内） --> 待機状態（エリア外）: ビーコン圏外へ
    note left of 待機状態（エリア外）
        エリア外では録画開始ボタンは無効化される。
    end note

    待機状態（エリア外） --> 待機状態（エリア内）: ビーコン圏内へ
```
...
stateDiagram-v2
    [*] --> 待機中
    待機中 --> アップロード中: 条件成立 (Wi-Fi, 電源等)
    アップロード中 --> 待機中: 転送完了 / 失敗 / 中断
```
...
- **安定性:** アプリケーションは24時間以上の連続稼働が可能であること。
- **ストレージ管理:** 録画ファイルは内蔵ストレージの専用ディレクトリに保存すること。
- **セキュリティ:** PCとのデータ転送は、同一セグメントのプライベートネットワーク内でのみ行われることを前提とする。
