...
- **物理ボタン割り当て:**
  - **ボタン2 (長押し):** 録画開始（待機状態時）、録画停止（録画中状態時）
- **状態遷移:**
...
- **物理ボタン割り当て:**
  - **待機状態（録画していない時）:**
    - **ボタン1 (短押し):** 音量を下げる。
    - **ボタン1 (長押し):** バッテリー残量、ネットワーク状態、ストレージ残量を順に音声通知する。
    - **ボタン2 (短押し):** 「現在録画していません。作業エリア（内／外）です」のように、エリア内外の状態も合わせて通知する。
    - **ボタン3 (短押し):** 音量を上げる。
    - **ボタン3 (長押し):** バッテリー残量、ネットワーク状態、ストレージ残量を順に音声通知する。
  - **録画中状態:**
    - **ボタン1 (長押し):** 録画詳細（例：「録画開始からxx秒経過しました」）を音声通知する。
    - **ボタン2 (短押し):** 「録画中です」と音声通知する。
    - **ボタン3 (長押し):** 録画詳細（例：「録画開始からxx秒経過しました」）を音声通知する。
    - **その他すべての短押し操作は無効**
- **自動通知トリガーと内容:**
  - **バッテリー残量低下:**
...
sequenceDiagram
    ...
    opt アップロード条件成立 (充電中 & Wi-Fi接続など)
        Device->>PC: ファイル転送を開始
        Note right of Device: LED点灯 (転送パターン)
        PC-->>Device: 転送完了
    ...
    Note right of Device: 状態: 録画中状態, LED点滅
    ...
    alt 通常の録画停止
        ...
        Note right of Device: 待機状態(エリア内)に転移, LED消灯
    else エリア外での強制録画停止
        User->>Device: (エリア外へ移動)
...
待機状態ではデバイス状態と録画状態を、録画中状態では録画状態のみが確認できます。これらの確認操作は本状態遷移に影響を与えません。

```mermaid
stateDiagram-v2
    [*] --> 待機状態（エリア内）: アプリ起動
    待機状態（エリア内） --> 録画中状態: ボタン2長押し
    録画中状態 --> 待機状態（エリア内）: ボタン2長押し
